...
- **EC2 インスタンス**
  - 仮想サーバOS：Amazon Linux 2023
- **ElasticIPの取得、割り当て**
- 固定IPアドレス発行、EC2に割り当て、ドメインと紐づけ
- **セキュリティグループの設定**
  - インバウンドルールを編集、HTTP(80)、HTTPS(443)、SSH(22)を開放
- **Route 53**
  - ドメイン管理のためホストゾーン作成、下記のDNSレコードをドメイン購入元に登録
  - Aレコード ：作成したドメインをEpastic.IPに紐づけ
  - CNAMEレコード ：www.looma.jpをlooma.jpに転送
  - NSレコード :NSをドメイン購入元からRoute53に指定
    -TXTレコード :SSL証明書の取得
- **Let's Encrypt（Certbot）**
  - Cli上でCertbotによりSSL証明書発行
- **AWS CLIでNginxの設定**
  - HTTPをリッスンしHTTPSへ転送設定
  - リバースプロキシサーバとしてHTTPSからPuma(127.0.0.1:3000)へ転送設定、SSL通信処理設定
  - 自動起動設定をsystemdにサービス登録
- **AWS CLIでPumaの設定**
  - 自動起動設定をsystemdにサービス登録
- **Amazon EventBridge**
  - 自動起動のロール、ルール設定
